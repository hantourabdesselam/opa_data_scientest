...
def save_model(model_prophet, symbol):
    path_save = 'data/serialized_model_for_'+str(symbol)+'_test.json'
    with open(path_save, 'w') as fout:
        fout.write(model_to_json(model_prophet))  # Save model
    return path_save

def load_model(symbol):
    with open('data/serialized_model_for_'+str(symbol)+'_test.json', 'r') as fin:
...
wallet = pd.DataFrame({'timestamp': [datetime.now()],'fonds_disponibles': [10000], 'BTC': [100], 'ETH': [0], 'DOGE': [0], 'fonds_investis':[0], 'total_actif':[0]})

symbol = 'BTC'
# symbol_price_csv_name = f"historical_prices_{symbol}.csv"
# symbol_price = pd.read_csv('data/'+symbol_price_csv_name, sep=';', encoding='utf-8')
# # print(symbol_price.head(5).to_dict('records'))
# symbol_model = run_training_model(symbol_price)
# path_save_model = save_model(symbol_model, symbol)
# if not os.path.exists(path_save_model):
#     print(f"File not found: {path_save_model}")
# else:
#     with open(path_save_model, 'r') as fin:
#         content = fin.read()
#         if not content:
#             print(f"File is empty: {path_save_model}")
#         else:
#             try:
#                 m = model_from_json(content)
#             except json.JSONDecodeError as e:
#                 print(f"JSON decode error: {e}")

new_wallet = get_action(wallet, 'BTC', 100, client, currency='EUR')
print(new_wallet.to_dict('records'))

update_wallet = get_updated_wallet(new_wallet, client)
print(update_wallet.to_dict('records'))
